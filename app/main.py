...
async def telemetry(
    ...
    if event_type == SALES_RECORDING_EVENT_TYPE:
        try:
            ...
            raise HTTPException(status_code=500, detail="Unable to store sales recording.") from exception
    ...
    # Other telemetry is intentionally best effort for forward compatibility.
...
async def _queue_sales_processing(attempt_id: str) -> None:
    ...
    def finished(completed: asyncio.Task[object]) -> None:
        ...
        # process_sales_attempt records expected processing failures. A task
        # exception here indicates an unexpected storage or scheduler error.
        try:
            completed.exception()
        except asyncio.CancelledError:
            return
        except Exception as exception:
            logger.error("Sales processing task failed for %s: %s", attempt_id, exception)

    task.add_done_callback(finished)


def _schedule_sales_processing(
    attempt_id: str, background_tasks: BackgroundTasks | None
) -> None:
...
async def submit_sales_persuasion_recording(
    ...
    # The client only waits for durable WAV and attempt metadata. Assessment
    # starts after the response is ready and does not hold up the encounter.
    if should_process or (
        record.get("assessmentStatus") == "processing"
...
class ConversationMessage(BaseModel):
    ...
    content: str = Field(min_length=1, max_length=REQUEST_SETTINGS.max_message_chars)
...
@app.post("/api/ai/initial-career-assessment", response_model=CareerAssessmentResponse)
        ...
        if parsed.action == "status":
            ...
            for status in statuses:
                if status.running:
                    ...
                    log(f"[AI] {status.name}: stopped (exit code {status.return_code}).")
                else:
